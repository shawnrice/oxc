...
impl VisitMut<'_> for SpecParser {
    ...
    fn visit_call_expression(&mut self, expr: &mut CallExpression<'_>) {
        ...
        // The `json` / `jsonc` / `json5` languages each accept only their own parser's calls.
        ...
        // The variant follows the language: `Json` -> `json`, `Jsonc` -> `jsonc`, `Json5` -> `json5`.
                    ...
                    match &obj_prop.value {
                        Expression::BooleanLiteral(literal) => {
                            ...
                            ) && literal.value
                            {
                                ...
                                // Only `json5` honors it; harmless for the other variants.
                                json_options.single_quote = literal.value;
